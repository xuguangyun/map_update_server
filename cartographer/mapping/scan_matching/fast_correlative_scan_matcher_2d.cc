...
PrecomputationGrid2D::PrecomputationGrid2D(
  ...
  score_index=(max_score_ - min_score_) / 255.f;
  score_index_rec = 1.f/score_index;
...
uint8 PrecomputationGrid2D::ComputeCellValue(const float probability) const {
  const int cell_value = common::RoundToInt(
      (probability - min_score_) * score_index_rec);
...
//  LOG(INFO)<<"spanA"<<timeSpanCalculate(&timeSystemStart);  //0  0
//  gettimeofday (&timeSystemStart , nullptr);
  const sensor::PointCloud rotated_point_cloud = sensor::TransformPointCloud(
      point_cloud,
      transform::Rigid3f::Rotation(Eigen::AngleAxisf(
          initial_rotation.cast<float>().angle(), Eigen::Vector3f::UnitZ())));
//  LOG(INFO)<<"spanB"<<timeSpanCalculate(&timeSystemStart);  //0  0
//  gettimeofday (&timeSystemStart , nullptr);
 ...
 // LOG(INFO)<<"spanC"<<timeSpanCalculate(&timeSystemStart);  //0  0
 // gettimeofday (&timeSystemStart , nullptr);
  const std::vector<Candidate2D> lowest_resolution_candidates =
      ComputeLowestResolutionCandidates(discrete_scans, search_parameters);
 // LOG(INFO)<<"spanD"<<timeSpanCalculate(&timeSystemStart);  //0  0
 // gettimeofday (&timeSystemStart , nullptr);
  const Candidate2D best_candidate = BranchAndBound(
      discrete_scans, search_parameters, lowest_resolution_candidates,
      precomputation_grid_stack_->max_depth(), min_score);
  //LOG(INFO)<<"spanE"<<timeSpanCalculate(&timeSystemStart);  //0  0
 // gettimeofday (&timeSystemStart , nullptr);
...
//          for (int candj=0;candj<discrete_scans[(*candidates)[candi].scan_index].size();++candj) {
//            const Eigen::Array2i proposed_xy_index(
//                discrete_scans[(*candidates)[candi].scan_index][candj].x() + (*candidates)[candi].x_index_offset,
//                discrete_scans[(*candidates)[candi].scan_index][candj].y() + (*candidates)[candi].y_index_offset);
//            int now=precomputation_grid.GetValue(proposed_xy_index);
//            sum += now;
//          }
//          (*candidates)[candi].score = precomputation_grid.ToScore(
//              sum / static_cast<float>(discrete_scans[(*candidates)[candi].scan_index].size()));
//        }
